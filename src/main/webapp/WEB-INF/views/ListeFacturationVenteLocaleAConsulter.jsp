...
<body ng-controller="ImportExportController"
	ng-init="getListVenteLocaleDejaFacture();">
	...
	<div id="general">
		...
		<div id="droite">
			...
			<div class="panel panel-primary">
				<div class="panel-heading">Liste des Ventes Locales</div>
				<div class="panel-body">
					<table class="table table-striped">
						<thead>
							<tr>
								<th scope="row">CODE ACHAT</th>
								<th scope="row">DATE ACHAT</th>
								<th scope="row">PRENOM CLIENT</th>
								<th scope="row">NOM CLIENT</th>
								<th scope="row">UTILISATEUR ENREGISTRE</th>
								<th scope="row">UTILISATEUR FACTURATION</th>
							</tr>
						</thead>
						<tr ng-repeat="p in listVenteLocale | filter:search">

							<td><a
								href="<%=request.getContextPath()%>/facturation/VenteLocale/GetFacturationVenteLocale?{{p.id}}">{{p.codeAchat}}</a></td>
							<td>{{p.dateAchat}}</td>
							<td>{{p.client.prenomClient}}</td>
