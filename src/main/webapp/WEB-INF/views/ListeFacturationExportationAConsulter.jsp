...
<body ng-controller="ImportExportController"
	ng-init="getListExportationDejaFacture()">
	...
	<div id="general">
		...
		<div class="panel panel-primary">
			<div class="panel-heading">Liste des Exportations</div>
			<div class="panel-body">
				<table class="table table-striped">
					<thead>
						<tr>
							<th scope="row">CODE EXPORTATION</th>
							<th scope="row">DATE EXPORTATION</th>
							<th scope="row">PRENOM DESTINATAIRE</th>
							<th scope="row">NOM DESTINATAIRE</th>
							<th scope="row">PAYS DE DESTINATION</th>
							<th scope="row">UTILISATEUR ENREGISTRE</th>
							<th scope="row">UTILISATEUR FACTURATION</th>
						</tr>
					</thead>
					<tr ng-repeat="p in listExportation | filter:search">
						<td><a
							href="<%=request.getContextPath()%>/facturation/exportation/GetFacturationExportation?{{p.id}}">{{p.codeExportation}}</a></td>
						<td>{{p.dateExportation}}</td>
						<td>{{p.destinataire.prenomDestinataire}}</td>
						<td>{{p.destinataire.nomDestinataire}}</td>
						<td>{{p.pays.pays}}</td>
						<td>{{p.utilisateur.nomComplet}}</td>
						<td>{{p.utilisateurFacturation.nomComplet}}</td>
